...
public enum CustomColors
{
    ...
    private static final CustomColors.IColorizer COLORIZER_GRASS = new CustomColors.IColorizer()
    {
        ...
        {
            BiomeGenBase biomegenbase = getColorBiome(blockAccess, blockPos);
    ...
    private static final CustomColors.IColorizer COLORIZER_WATER = new CustomColors.IColorizer()
    {
        ...
        {
            BiomeGenBase biomegenbase = getColorBiome(blockAccess, blockPos);
            return waterColors != null ? waterColors.getColor(biomegenbase, blockPos) : (biomegenbase.waterColorMultiplier);
        }
        public boolean isColorConstant()
        {
            return false;
    ...
    public static CustomColormap getCustomColors(String pathImage, int width, int height)
    {
        try
        {
            ...
            else
            {
                ...
                {
                    properties.setProperty("format", paletteFormatDefault);
                    properties.setProperty("source", pathImage);
                    s = pathImage;
                }
    ...
    private static float[][] readDyeColors(Properties props, String fileName, String prefix, String logName)
    {
        ...
        for (Object o : props.keySet())
        {
            ...
            if (s.startsWith(prefix))
            {
                ...
                {
                    float[] afloat = {(j >> 16 & 255) / 255.0F, (j >> 8 & 255) / 255.0F, (j & 255) / 255.0F};
    ...
    {
        return name == null ? -1 : ("air".equals(name) ? MapColor.airColor.colorIndex : ("grass".equals(name) ? MapColor.grassColor.colorIndex : ("sand".equals(name) ? MapColor.sandColor.colorIndex : ("cloth".equals(name) ? MapColor.clothColor.colorIndex : ("tnt".equals(name) ? MapColor.tntColor.colorIndex : ("ice".equals(name) ? MapColor.iceColor.colorIndex : ("iron".equals(name) ? MapColor.ironColor.colorIndex : ("foliage".equals(name) ? MapColor.foliageColor.colorIndex : ("clay".equals(name) ? MapColor.clayColor.colorIndex : ("dirt".equals(name) ? MapColor.dirtColor.colorIndex : ("stone".equals(name) ? MapColor.stoneColor.colorIndex : ("water".equals(name) ? MapColor.waterColor.colorIndex : ("wood".equals(name) ? MapColor.woodColor.colorIndex : ("quartz".equals(name) ? MapColor.quartzColor.colorIndex : ("gold".equals(name) ? MapColor.goldColor.colorIndex : ("diamond".equals(name) ? MapColor.diamondColor.colorIndex : ("lapis".equals(name) ? MapColor.lapisColor.colorIndex : ("emerald".equals(name) ? MapColor.emeraldColor.colorIndex : ("podzol".equals(name) ? MapColor.obsidianColor.colorIndex : ("netherrack".equals(name) ? MapColor.netherrackColor.colorIndex : (!"snow".equals(name) && !"white".equals(name) ? (!"adobe".equals(name) && !"orange".equals(name) ? ("magenta".equals(name) ? MapColor.magentaColor.colorIndex : (!"light_blue".equals(name) && !"lightBlue".equals(name) ? ("yellow".equals(name) ? MapColor.yellowColor.colorIndex : ("lime".equals(name) ? MapColor.limeColor.colorIndex : ("pink".equals(name) ? MapColor.pinkColor.colorIndex : ("gray".equals(name) ? MapColor.grayColor.colorIndex : ("silver".equals(name) ? MapColor.silverColor.colorIndex : ("cyan".equals(name) ? MapColor.cyanColor.colorIndex : ("purple".equals(name) ? MapColor.purpleColor.colorIndex : ("blue".equals(name) ? MapColor.blueColor.colorIndex : ("brown".equals(name) ? MapColor.brownColor.colorIndex : ("green".equals(name) ? MapColor.greenColor.colorIndex : ("red".equals(name) ? MapColor.redColor.colorIndex : ("black".equals(name) ? MapColor.blackColor.colorIndex : -1)))))))))))) : MapColor.lightBlueColor.colorIndex)) : MapColor.adobeColor.colorIndex) : MapColor.snowColor.colorIndex)))))))))))))))))))));
    }
